...
import { CONFIG, NODE_TYPES, heatForMove } from './config.js';
...
export class Renderer {
  ...
  applyNode(el, node, { instant = false } = {}) {
    ...
    const specialText = node.special
      ? `, line breaker ${node.special === 'lineH' ? 'horizontal' : 'vertical'}`
      : '';
  ...
  /** Discharge beams for activated Line Breakers. */
  showBeams(activations) {
    if (!activations.length) return;
    const geo = this.geometry();
    if (!geo) return;
    const frag = document.createDocumentFragment();
    activations.forEach(({ row, col, special }) => {
      const beam = document.createElement('i');
      beam.className = special === 'lineV' ? 'beam beam-v' : 'beam';
