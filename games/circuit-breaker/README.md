...
- An **escalating heat curve** — moves cost +4 early and up to +8 late, so the
  opening breathes and the endgame tightens (the HUD shows the next move's cost)
- Three **special nodes**, forged by the shape of your match and chaining into
  one another:
  - **Line Breaker** (four in a line) — discharges that whole row or column
  - **Pulse Bomb** (an L or T shape) — discharges a 3×3 area
  - **Wildcard Core** (five in a line) — swap it onto any node to discharge
    every node of that type; two Cores swapped together wipe the grid
...
tools/serve.mjs          dev server (optionally under a sub-path)
tools/simulate.mjs       headless game simulation, shared by the tools below
tools/test-logic.mjs     headless model tests
tools/tune-pacing.mjs    plays hundreds of games per heat curve and compares them
tools/measure-audio.js   renders every sound offline and reports its levels
```
...
**Heat curve.** `HEAT_PER_VALID_MOVE` is the opening cost; every
`HEAT_RAMP_EVERY` moves it rises by `HEAT_RAMP_STEP` up to `HEAT_PER_MOVE_MAX`.
Shipping values are +4 for the first 10 moves, then +1 per 10 moves, capped at
+8. Set `HEAT_RAMP_STEP: 0` for a flat curve.

Before changing any of it, run the sweep — it plays hundreds of complete games
per candidate curve and prints run length, score, and how often Line Breakers
actually get used:

```bash
node tools/tune-pacing.mjs 250
```

Measured with the shipping values, all three specials enabled:

| play     | moves | score  | specials forged |
|----------|-------|--------|-----------------|
| careless | 24.6  | 5,349  | 2.6 / run       |
| skilled  | 42.7  | 22,788 | ~18 / run       |

Playing for cooling roughly **doubles run length and quadruples score**.
`npm test` asserts these stay in a sane band, so a future retune can't quietly
make runs trivially short or endless, and checks that every special type is
still both forged and fired during ordinary play.
...
- A single match shape forges at most one special, choosing the rarest it
  qualifies for (five-line → crossing → four-line). This is deliberate: letting
  one L-shaped match spawn two specials made big shapes wildly swingy.
- Sound levels are verified objectively (`tools/measure-audio.js` confirms no
  clipping, a sane loudness hierarchy, and safe headroom when several sounds
  overlap) but the *timbre* has not been auditioned on a real device. If a
  sound feels wrong, adjust its `gain` in the `VOICES` table in `js/audio.js`
  and re-measure.

## Future ideas

Daily seeded challenge, limited-move and timed modes, obstacles or corrupted
nodes, unlockable themes, achievements, and a local statistics dashboard.

Adding another special is a small job: classify the shape in `planSpecials()`
(`js/matches.js`) and give it a blast footprint in `Board.blastCells()`
(`js/board.js`). Chaining, scoring, cooling and the activation queue all work
off those two functions, so nothing else needs to change.
