...
import {
  findMatchGroups, groupCells, planSpecials, expandActivations, planWildcardSwap,
} from './matches.js';
...
import { key, wait, waitFor, duration, clearAllTimers, formatNumber } from './utils.js';
...
export class Game {
  ...
  async attemptSwap(a, b) {
    ...
    try {
      // A Wildcard Core discharges on contact, so its swap is legal even when
      // it lines nothing up.
      const wildcard = planWildcardSwap(this.board, a, b);

      if (!wildcard && !this.board.swapCreatesMatch(a, b)) {
   ...
   *
   * `seed` forces a first step with no match behind it — that is how a Wildcard
   * swap discharges.
   */
  async resolve(run, originCells, seed = null) {
    ...
    for (;;) {
      ...
      step++;
      let matched = groupCells(groups);
      let overrides = null;
      if (seedNow) {
        const seedKey = key(seedNow.row, seedNow.col);
        if (!matched.some((c) => key(c.row, c.col) === seedKey)) {
          matched = [...matched, { row: seedNow.row, col: seedNow.col }];
        }
        overrides = new Map([[seedKey, seedNow.targetType]]);
      }
      ...
      const { cells, activations, keys } = expandActivations(this.board, matched, overrides);
      if (activations.length) {
        this.renderer.showBlasts(activations);
