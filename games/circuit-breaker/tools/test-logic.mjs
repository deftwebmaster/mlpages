...
import { CONFIG, SPECIAL, heatForMove } from '../js/config.js';
import { simulateGame } from './simulate.mjs';
...
test('a plain three-run forges nothing', () => {
  const three = plant(basePattern(), hRun(0, 0, 2), 'B');
  assert.equal(planSpecials(findMatchGroups(three.grid), []).length, 0);
});

test('a five-run forges a Wildcard Core', () => {
  const board = plant(basePattern(), hRun(2, 1, 5), 'B');
  const groups = findMatchGroups(board.grid);
  assert.equal(groups.length, 1);
  assert.equal(groups[0].length, 5);
  const plans = planSpecials(groups, []);
  assert.equal(plans.length, 1);
  assert.equal(plans[0].special, SPECIAL.WILDCARD);
  assert.equal(plans[0].type, 'B');
  // No moved cell → centre-most cell of the run.
  assert.deepEqual({ row: plans[0].row, col: plans[0].col }, { row: 2, col: 3 });
});

test('an L/T intersection forges a Pulse Bomb at the crossing', () => {
  const board = basePattern();
  plant(board, hRun(2, 1, 3), 'B');
  plant(board, vRun(2, 2, 4), 'B');   // crosses the horizontal arm at (2,2)
  const groups = findMatchGroups(board.grid);
  assert.equal(groups.length, 2);
  const plans = planSpecials(groups, []);
  assert.equal(plans.length, 1, 'one shape must yield exactly one special');
  assert.equal(plans[0].special, SPECIAL.BOMB);
  assert.deepEqual({ row: plans[0].row, col: plans[0].col }, { row: 2, col: 2 });
});

test('special priority: five-line beats a crossing, a crossing beats four-line', () => {
  // A five-run crossed by a three-run: the rarer Wildcard wins, and only one
  // special is forged for the whole shape.
  const wild = basePattern();
  plant(wild, hRun(3, 1, 5), 'B');
  plant(wild, vRun(3, 2, 4), 'B');
  const wildPlans = planSpecials(findMatchGroups(wild.grid), []);
  assert.equal(wildPlans.length, 1);
  assert.equal(wildPlans[0].special, SPECIAL.WILDCARD);

  // A four-run crossed by a three-run: the crossing wins over the Line Breaker.
  const bomb = basePattern();
  plant(bomb, hRun(3, 1, 4), 'B');
  plant(bomb, vRun(3, 2, 4), 'B');
  const bombPlans = planSpecials(findMatchGroups(bomb.grid), []);
  assert.equal(bombPlans.length, 1);
  assert.equal(bombPlans[0].special, SPECIAL.BOMB);
});

test('two separate shapes each forge their own special', () => {
  const board = basePattern();
  plant(board, hRun(0, 0, 3), 'B');   // four in a row  → Line Breaker
  plant(board, hRun(5, 1, 5), 'C');   // five in a row  → Wildcard
  const plans = planSpecials(findMatchGroups(board.grid), []);
  assert.equal(plans.length, 2);
  assert.deepEqual(plans.map((p) => p.special).sort(), [SPECIAL.LINE_H, SPECIAL.WILDCARD].sort());
});

test('a Pulse Bomb discharges a 3x3, clamped at the board edge', () => {
  const board = plant(basePattern(), hRun(3, 0, 2), 'C');
  board.grid[3][1].special = SPECIAL.BOMB;
  const { cells, activations } = expandActivations(board, groupCells(findMatchGroups(board.grid)));
  assert.equal(activations.length, 1);
  assert.equal(activations[0].special, SPECIAL.BOMB);
  // Rows 2-4 x cols 0-2 is nine cells; the matched run is inside that block.
  assert.equal(cells.length, 9);
  const ids = cellKeys(cells);
  assert.ok(ids.includes('2,0') && ids.includes('4,2'));

  // A bomb in the corner clears only the four cells that exist.
  const corner = plant(basePattern(), hRun(0, 0, 2), 'C');
  corner.grid[0][0].special = SPECIAL.BOMB;
  const cornerBlast = expandActivations(corner, groupCells(findMatchGroups(corner.grid)));
  assert.deepEqual(cellKeys(cornerBlast.cells).sort(), ['0,0', '0,1', '0,2', '1,0', '1,1'].sort());
});

test('a Wildcard Core discharges every node of its own type when matched', () => {
  const board = plant(basePattern(), hRun(3, 0, 2), 'C');
  board.grid[3][1].special = SPECIAL.WILDCARD;
  const expectedC = [];
  board.forEachNode((node, r, c) => { if (node.type === 'C') expectedC.push(`${r},${c}`); });

  const { cells, activations } = expandActivations(board, groupCells(findMatchGroups(board.grid)));
  assert.equal(activations.length, 1);
  assert.equal(activations[0].special, SPECIAL.WILDCARD);
  assert.deepEqual(cellKeys(cells), expectedC.sort(), 'exactly the C nodes should go');
  assert.ok(cells.length > 3, 'a Wildcard should reach well beyond its own match');
});

test('a Wildcard swapped onto a node consumes that node type', () => {
  const board = basePattern();
  board.grid[3][3].special = SPECIAL.WILDCARD;
  const targetType = board.grid[3][4].type;
  const expected = [];
  board.forEachNode((node, r, c) => {
    if (node.type === targetType || (r === 3 && c === 3)) expected.push(`${r},${c}`);
  });

  // The swap puts the core where the standard node was.
  board.swap({ row: 3, col: 3 }, { row: 3, col: 4 });
  const seed = { row: 3, col: 4 };
  const overrides = new Map([['3,4', targetType]]);
  const { cells, activations } = expandActivations(board, [seed], overrides);

  assert.equal(activations.length, 1);
  assert.equal(activations[0].targetType, targetType);
  assert.equal(cells.length, expected.length);
  assert.ok(cells.every(({ row, col }) => {
    const node = board.at(row, col);
    return node.type === targetType || node.special === SPECIAL.WILDCARD;
  }));
});

test('two Wildcards swapped together wipe the board', () => {
  const board = basePattern();
  board.grid[3][3].special = SPECIAL.WILDCARD;
  board.grid[3][4].special = SPECIAL.WILDCARD;
  const overrides = new Map([['3,4', '*']]);
  const { cells } = expandActivations(board, [{ row: 3, col: 4 }], overrides);
  assert.equal(cells.length, board.rows * board.cols, 'every node should be cleared');
});

test('specials chain into each other across all three types', () => {
  const board = plant(basePattern(), hRun(3, 0, 2), 'C');
  board.grid[3][1].special = SPECIAL.LINE_H;  // matched → clears row 3
  board.grid[3][5].special = SPECIAL.BOMB;    // caught by row 3 → clears 3x3 around (3,5)
  const { cells, activations } = expandActivations(board, groupCells(findMatchGroups(board.grid)));

  assert.equal(activations.length, 2);
  assert.deepEqual(activations.map((a) => a.special), [SPECIAL.LINE_H, SPECIAL.BOMB]);
  const ids = new Set(cellKeys(cells));
  assert.equal(ids.size, cells.length, 'cleared cells must be unique');
  // Row 3 (7) plus rows 2 and 4 at cols 4-6 (6 more) = 13.
  assert.equal(cells.length, 13);
  assert.ok(ids.has('2,4') && ids.has('4,6'));

  board.clearCells(cells);
  board.collapse();
  assertBoardFull(board, 'line breaker into bomb');
});

test('a bomb chain cannot loop or double-clear', () => {
  const board = plant(basePattern(), hRun(3, 0, 2), 'C');
  // A dense cluster of bombs that all reach each other.
  [[2, 1], [3, 1], [4, 1], [3, 2]].forEach(([r, c]) => { board.grid[r][c].special = SPECIAL.BOMB; });
  const { cells, activations } = expandActivations(board, groupCells(findMatchGroups(board.grid)));
  assert.equal(activations.length, 4, 'each bomb fires exactly once');
  assert.equal(new Set(cellKeys(cells)).size, cells.length);
  board.clearCells(cells);
  board.collapse();
  assertBoardFull(board, 'bomb cluster');
});
...
test('the heat ramp starts gentle, steps up, and caps', () => {
  assert.equal(heatForMove(1), CONFIG.HEAT_PER_VALID_MOVE);
  assert.equal(heatForMove(CONFIG.HEAT_RAMP_EVERY), CONFIG.HEAT_PER_VALID_MOVE);
  assert.equal(heatForMove(CONFIG.HEAT_RAMP_EVERY + 1),
    CONFIG.HEAT_PER_VALID_MOVE + CONFIG.HEAT_RAMP_STEP);
  assert.equal(heatForMove(10_000), CONFIG.HEAT_PER_MOVE_MAX);

  // Never decreases, never exceeds the cap.
  let previous = 0;
  for (let move = 1; move <= 500; move++) {
    const rate = heatForMove(move);
    assert.ok(rate >= previous, `rate dropped at move ${move}`);
    assert.ok(rate <= CONFIG.HEAT_PER_MOVE_MAX, `rate exceeded the cap at move ${move}`);
    previous = rate;
  }
});
...
/* --------------------------------------------------------------------------
   ...
   (shared with tools/tune-pacing.mjs via tools/simulate.mjs)
   -------------------------------------------------------------------------- */

/** Invariant checker handed to the simulation on every resolution step. */
function checkInvariants({ board, cells, step, turn, phase }) {
  if (phase === 'before-clear') {
    const unique = new Set(cells.map((c) => `${c.row},${c.col}`));
    assert.equal(unique.size, cells.length, `turn ${turn} step ${step}: duplicate cell in clear set`);
    cells.forEach(({ row, col }) => {
      assert.ok(board.at(row, col), `turn ${turn} step ${step}: clearing an empty cell`);
    });
  } else {
    assertBoardFull(board, `turn ${turn} step ${step}`);
  }
}
...
test('200 simulated games keep the board consistent and always reach overload', () => {
  ...
  for (let i = 0; i < 200; i++) {
    const { stats, heat } = simulateGame({ strategy: 'first', validate: checkInvariants });
    totals.games++;
...
test('all three special types get forged and fired in real play', () => {
  const created = {};
  const fired = {};
  for (let i = 0; i < 120; i++) {
    const { stats } = simulateGame({ strategy: 'greedy', validate: checkInvariants });
    Object.entries(stats.createdByType).forEach(([k, n]) => { created[k] = (created[k] || 0) + n; });
    Object.entries(stats.byType).forEach(([k, n]) => { fired[k] = (fired[k] || 0) + n; });
  }

  process.stdout.write(
    `        forged  ${JSON.stringify(created)}\n        fired   ${JSON.stringify(fired)}\n`,
  );

  [SPECIAL.LINE_H, SPECIAL.LINE_V, SPECIAL.BOMB, SPECIAL.WILDCARD].forEach((kind) => {
    assert.ok(created[kind] > 0, `${kind} was never forged in 120 games`);
    assert.ok(fired[kind] > 0, `${kind} was never activated in 120 games`);
  });
});

test('pacing: a careless run lasts long enough to see the mechanics', () => {
  const runs = 80;
  let moves = 0;
  let breakersMade = 0;
  for (let i = 0; i < runs; i++) {
    const { stats } = simulateGame({ strategy: 'first' });
    moves += stats.turns;
    breakersMade += stats.specialsCreated;
  }
  const avgMoves = moves / runs;
  const avgBreakers = breakersMade / runs;

  process.stdout.write(
    `        careless: ${avgMoves.toFixed(1)} moves/run · ${avgBreakers.toFixed(2)} breakers forged/run\n`,
  );

  // Guards against a future retune quietly making runs trivially short or endless.
  assert.ok(avgMoves >= 18, `careless runs are too short (${avgMoves.toFixed(1)} moves)`);
  assert.ok(avgMoves <= 45, `careless runs are too long (${avgMoves.toFixed(1)} moves)`);
  assert.ok(avgBreakers >= 1, `players rarely even see a Line Breaker (${avgBreakers.toFixed(2)}/run)`);
});

test('skilled play is rewarded: chasing cooling beats taking the first move', () => {
  const runs = (strategy, count) => {
    const out = { moves: 0, score: 0 };
    for (let i = 0; i < count; i++) {
      const { stats } = simulateGame({ strategy });
  ...
  const naive = runs('first', 50);
  const skilled = runs('greedy', 50);
  ...
  assert.ok(skilled.moves > naive.moves * 1.3,
