...
export class Renderer {
  ...
  showAdBreak(sponsor, breakNumber) {
    this.elements.adTitle.textContent = sponsor.name;
    this.elements.adCopy.textContent = sponsor.copy;
    this.elements.adDisclaimer.textContent = sponsor.disclaimer;
    this.elements.adOverlay.dataset.variant = String((breakNumber % 3) + 1);
    this.showOverlay("ad", true);
  }

  updateMonetization(settings, movesUntilAd = 0) {
    const adsBypassed = Boolean(settings.adsBypassed);
    if (this.elements.menuPurchaseButton) {
      this.elements.menuPurchaseButton.textContent = adsBypassed ? "Ads Removed" : "Remove Ads $0.00";
      this.elements.menuPurchaseButton.disabled = adsBypassed;
    }
    if (this.elements.menuAdStatus) {
      this.elements.menuAdStatus.textContent = adsBypassed
        ? "Premium satire active: fake ads are bypassed"
        : "Fake ads enabled for maximum shareholder whimsy";
    }
    if (this.elements.sponsorText) {
      const moveLabel = movesUntilAd === 1 ? "move" : "moves";
      this.elements.sponsorText.textContent = adsBypassed
        ? "Ads removed via pretend purchase"
        : `${Math.max(1, movesUntilAd)} ${moveLabel} until fake ad`;
    }
  }
  ...
  showGameOver(results, isNewBest) {
    const rows = [
      ...
      ["Fake ads endured", formatNumber(results.fakeAdsWatched)],
  ...
  syncToggles(settings) {
    ...
    this.updateMonetization(settings);
