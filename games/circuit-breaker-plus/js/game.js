...
export class CircuitBreakerGame {
  ...
  resetRunData() {
    ...
    this.adBreaksSeen = 0;
    this.nextAdMove = CONFIG.ads.firstBreakAfterMoves;
  }

  init() {
    this.renderer.syncToggles(this.settings);
    this.renderer.updateMonetization(this.settings, this.movesUntilNextAd());
  ...
  returnToMenu() {
    ...
    this.renderer.showOverlay("ad", false);
    this.renderer.showOverlay("purchase", false);
  ...
  showPurchase() {
    this.audio.play("button");
    this.renderer.showOverlay("purchase", true);
  }
  ...
  completeFakePurchase(message) {
    this.audio.play("button");
    this.settings.adsBypassed = true;
    saveSetting("adsBypassed", true);
    this.renderer.showOverlay("purchase", false);
    this.renderer.showOverlay("ad", false);
    this.renderer.updateMonetization(this.settings, this.movesUntilNextAd());
    this.renderer.setRunStatus(message);
    if (this.state === CONFIG.states.adBreak) {
      this.postAdStatus = message;
      this.resolveAdBreak();
    }
  }
  ...
  async resolveBoard(movedCell = null) {
    ...
    if (this.shouldShowAdBreak()) {
      await this.showAdBreak();
    }

    this.setState(CONFIG.states.playerTurn);
    this.renderer.setRunStatus(this.postAdStatus || this.statusForHeat());
    this.postAdStatus = "";
    this.renderer.updateStats(this.snapshot());
    this.renderer.updateMonetization(this.settings, this.movesUntilNextAd());
  }
  ...
  shouldShowAdBreak() {
    return !this.settings.adsBypassed && this.stats.validMoves >= this.nextAdMove;
  }

  showAdBreak() {
    this.setState(CONFIG.states.adBreak);
    const sponsor = CONFIG.ads.sponsors[this.adBreaksSeen % CONFIG.ads.sponsors.length];
    this.adBreaksSeen += 1;
    this.stats.fakeAdsWatched += 1;
    this.nextAdMove += CONFIG.ads.movesBetweenBreaks;
    this.renderer.updateMonetization(this.settings, this.movesUntilNextAd());
    this.renderer.setRunStatus(`Transmission interrupted by ${sponsor.name}.`);
    this.renderer.showAdBreak(sponsor, this.adBreaksSeen);
    this.audio.play("warning");
    this.audio.vibrate(18);

    return new Promise((resolve) => {
      this.adBreakResolver = resolve;
    });
  }

  skipAdBreak() {
    if (this.state !== CONFIG.states.adBreak) return;
    this.audio.play("button");
    this.renderer.showOverlay("ad", false);
    this.renderer.setRunStatus("Ad skipped. The imaginary economy trembles.");
    this.postAdStatus = "Ad skipped. The imaginary economy trembles.";
    this.resolveAdBreak();
  }

  resolveAdBreak() {
    const resolve = this.adBreakResolver;
    this.adBreakResolver = null;
    if (resolve) resolve();
  }
  ...
  movesUntilNextAd() {
    if (this.settings.adsBypassed) return 0;
    return Math.max(1, this.nextAdMove - this.stats.validMoves);
  }
